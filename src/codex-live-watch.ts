...
type LogSource = 'commands' | 'output' | 'timeline' | 'unknown';
...
function highlightAlertWords(line: string): string {
  return line.replace(/\b(ALERTA|WARN|WARNING|ERROR|FAIL)\b/gi, (m) => warn(m));
}

function paintStandardLogLine(raw: string, source: Exclude<LogSource, 'timeline'>): string {
  const line = raw.replace(/\r/g, '');
  if (!line) return line;

  if (/exit=[1-9]/.test(line)) return fail(line);
  if (/erro:|error:|exception|traceback/i.test(line)) return fail(line);
  if (/ALERTA|WARN|warning|missing|fail/i.test(line)) return highlightAlertWords(line);

  const lineWithModuleColors = highlightModules(line);
  if (lineWithModuleColors !== line) return highlightAlertWords(lineWithModuleColors);

  if (line.startsWith('[RUN]')) return dodgeBlue(highlightAlertWords(lineWithModuleColors));
  if (/^\[[0-9]{4}-[0-9]{2}-[0-9]{2}T/.test(line) && line.includes('$ ')) return cmd(line);
  if (line.includes('exit=0')) return ok(line);
  if (/^\s*etapa[_ -][0-9]+/i.test(line) || line.includes('etapa_')) return stage(highlightAlertWords(lineWithModuleColors));
  if (/\.jsonl?$/.test(line) || line.includes('/sessions/') || line.includes('\\sessions\\')) return file(lineWithModuleColors);

  if (source === 'commands') return dim(highlightAlertWords(lineWithModuleColors));
...
function renderTimelineLine(raw: string): string[] {
  const line = raw.replace(/\r/g, '');
  if (!line) return [];

  const m = line.match(/^\[([^\]]+)\]\s+\[(CMD|INFO|OUT|EXIT)\]\s*(.*)$/);
  if (!m) return [paintStandardLogLine(line, 'output')];
  ...
  const decorate = (s: string) => highlightAlertWords(highlightModules(s));
...
async function main(): Promise<number> {
  ...
  let currentSource: LogSource = hasTimeline ? 'timeline' : 'unknown';
  const mapSourceFromHeader = (line: string): LogSource => {
    if (line.includes('timeline.log')) return 'timeline';
