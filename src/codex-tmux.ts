...
type WatchMode = 'popup' | 'split' | 'both' | 'window' | 'none';
...
function parseArgs(argv: string[]): Options {
  ...
  while (args.length > 0) {
    ...
    if (a === '--repo') {
      const v = args.shift();
      if (!v) throw new Error('--repo exige valor');
      out.repo = v;
      continue;
    }
    if (a === '--no-popup') {
    ...
    if (a === '--popup-width' || a === '--width') {
      const v = args.shift();
      if (!v) throw new Error(`${a} exige valor`);
      out.popupWidth = v;
      continue;
    }
    ...
    if (a === '--log-dir') {
      const v = args.shift();
      if (!v) throw new Error('--log-dir exige valor');
      out.logEnabled = true;
      out.logDir = v;
      continue;
    }
    if (a === '--log-file') {
      const v = args.shift();
      if (!v) throw new Error('--log-file exige valor');
      out.logEnabled = true;
      out.logFile = v;
      continue;
    }
...
class JsonLogger {
  ...
  private watchAuditFile = '';
  private finalStatus: Status = 'ok';

  constructor(enabled: boolean, args: string[], logDir: string, logFile?: string) {
  ...
  getWatchAuditFile(): string {
    return this.watchAuditFile;
  }

  log(step: string, status: Status, message: string, details = ''): void {
    ...
    if (status === 'fail') this.finalStatus = 'fail';
    if (status === 'warn' && this.finalStatus !== 'fail') this.finalStatus = 'warn';
  }

  write(exitCode: number, options: Options): void {
    if (!this.enabled) return;
...
function setHook(session: string, hook: string, value?: string): void {
  if (!value) {
...
function launchAttachInNewTerminal(session: string, logger: JsonLogger): boolean {
...
function spawnPopupOnAttach(session: string, width: string, height: string, watchCmd: string, logger: JsonLogger): void {
  const worker =
...
function windowExists(windowId: string): boolean {
  ...
  if (windows.code !== 0) return false;
...
function clearWatchSplit(session: string, logger: JsonLogger): void {
  const existing = tmux(['show-options', '-t', session, '-qv', '@watch_pane']).out;
  if (existing && paneExists(existing)) {
    spawnSync('tmux', ['kill-pane', '-t', existing], { stdio: 'ignore' });
...
function clearWatchWindow(session: string, logger: JsonLogger): void {
  const existing = tmux(['show-options', '-t', session, '-qv', '@watch_window']).out;
  if (existing && windowExists(existing)) {
    spawnSync('tmux', ['kill-window', '-t', existing], { stdio: 'ignore' });
...
function main(): number {
  ...
  let exitCode = 0;
  try {
    if (!commandExists('tmux')) throw new Error('tmux não encontrado');
