...
function findSessionCatalogEntryById(sessionId) {
...
function resolveCodexWatchTarget(cfg, opts, rawTarget) {
    ...
    if (fs.existsSync(raw) && fs.statSync(raw).isFile())
        return raw;
    if (/^\d+$/.test(raw))
        return resolveCodexSessionNumber(raw);
    return findCodexSessionId(raw) ?? raw;
}
function ensureNoSessionSelector(action, opts) {
...
function readFileTailLines(filePath, lines) {
...
function extractSearchTerms(memory) {
    ...
    const push = (value, kind) => {
        ...
        if (!clean || clean.length < 2)
            return;
        if (kind === 'token' && (clean.length < 3 || SEARCH_STOPWORDS.has(clean)))
            return;
...
function inferSearchFromMemory(memory) {
    ...
    if (unit === 'h' || unit === 'hora' || unit === 'horas')
        return Date.now() - amount * 3600 * 1000;
    if (unit === 'd' || unit === 'dia' || unit === 'dias')
        return Date.now() - amount * 86400 * 1000;
    if (unit === 'w' || unit === 'semana' || unit === 'semanas')
        return Date.now() - amount * 7 * 86400 * 1000;
    return Date.now() - amount * 30 * 86400 * 1000;
}
function deepSearchFileHitsForTerms(terms) {
    const out = new Map();
    if (!commandExists('rg'))
        return out;
...
function recordSnippetFromJsonLine(rawLine) {
...
function readContentEvidence(filePath, term) {
    if (!commandExists('rg'))
        return null;
    const res = execCapture('rg', ['-n', '-i', '-F', '-m', '12', term, filePath], { stdio: ['ignore', 'pipe', 'ignore'] });
    if (res.code !== 0)
        return null;
    ...
    for (const line of lines) {
        ...
        const evidence = { term, source: 'content', snippet };
        if (!fallback)
            fallback = evidence;
...
function repoMatchesFilter(row, repoDir) {
    if (!repoDir)
        return true;
    if (!row.repoDir)
        return false;
...
function buildSearchCandidates(query) {
    ...
    if (query.fromMs !== null)
        rows = rows.filter((row) => row.startedAtMs >= query.fromMs);
    if (query.toMs !== null)
        rows = rows.filter((row) => row.startedAtMs <= query.toMs);
    const nowMs = Date.now();
    const candidates = rows
        ...
        if (query.inferredFromMs !== null) {
            if (row.startedAtMs >= query.inferredFromMs)
                score += 10;
            else
                score -= 2;
        }
    ...
    for (const candidate of top) {
        for (const term of candidate.matchedTerms) {
            if (candidate.evidences.some((e) => e.term === term && e.source === 'content'))
                continue;
            const contentEvidence = readContentEvidence(candidate.row.dirPath, term);
            if (contentEvidence)
                candidate.evidences.push(contentEvidence);
        }
...
function parseSearchQueryOptions(args, cfg, opts) {
    ...
    for (let i = 0; i < args.length; i += 1) {
        ...
        if (a === '--ask') {
            ...
            if (!prompt)
                throw new Error('--ask requer uma pergunta entre aspas');
            setAction('ask');
            askPrompt = prompt;
            i += 1;
            continue;
        ...
        if (a === '--limit') {
            ...
            if (!Number.isFinite(n) || n < 1)
                throw new Error(`--limit inválido: ${args[i + 1] ?? ''}`);
            limit = Math.floor(n);
            i += 1;
            continue;
        }
        if (a === '--candidate-limit') {
            const n = Number(args[i + 1] ?? '');
            if (!Number.isFinite(n) || n < 1)
                throw new Error(`--candidate-limit inválido: ${args[i + 1] ?? ''}`);
        ...
        if (a === '--hours') {
            ...
            if (!Number.isFinite(n) || n <= 0)
                throw new Error(`--hours inválido: ${args[i + 1] ?? ''}`);
        ...
        if (a === '--days') {
            const n = Number(args[i + 1] ?? '');
            if (!Number.isFinite(n) || n <= 0)
                throw new Error(`--days inválido: ${args[i + 1] ?? ''}`);
        ...
        if (a === '--weeks') {
            const n = Number(args[i + 1] ?? '');
            if (!Number.isFinite(n) || n <= 0)
                throw new Error(`--weeks inválido: ${args[i + 1] ?? ''}`);
        ...
        if (a === '--months') {
            const n = Number(args[i + 1] ?? '');
            if (!Number.isFinite(n) || n <= 0)
                throw new Error(`--months inválido: ${args[i + 1] ?? ''}`);
...
function buildSearchJsonCandidates(candidates) {
...
function buildCodexSearchPrompt(memory, query, terms, candidates) {
...
function runCodexSearchDecision(memory, query, terms, candidates) {
    ...
    const res = execCaptureInput('codex', [
        'exec',
        '--ephemeral',
        '--skip-git-repo-check',
        '--color',
...
function resolveSearchSelectedSession(candidates, codexDecision) {
...
function defaultSearchActionRepo(cfg, query, row) {
    return query.repoDir || row?.repoDir || resolveRepo(BASE_DIR, cfg);
}
async function openSearchSelectedSession(cfg, query, sessionId, row) {
    const repo = defaultSearchActionRepo(cfg, query, row);
    const callArgs = ['--repo', repo, '--', 'codex', 'resume', sessionId];
    console.log('');
...
function runCodexSearchAsk(cfg, query, sessionId, row, prompt) {
    if (!commandExists('codex'))
        throw new Error('comando `codex` não encontrado no PATH');
    const repo = defaultSearchActionRepo(cfg, query, row);
    const searchDir = searchLogsRoot();
...
async function cmdFlow(args) {
    ...
    const callArgs = ['--repo', repo, '--', ...cmdLine];
...
async function cmdMonitor(action, args) {
    ...
    if (action === 'tmux') {
        syncTmuxConfCopy();
        const repo = resolveRepo(BASE_DIR, cfg, opts.repo);
        const callArgs = ['--watch-target', watchTarget, '--repo', repo];
        if (opts.width)
...
async function cmdSearch(args) {
    ...
    if (query.action === 'ask') {
        if (!selected.sessionId)
            throw new Error('nenhuma sessão encontrada para `--ask`');
    ...
    if (query.repoDir)
        console.log(`  repo_filter=${file(query.repoDir)}`);
    if (terms.length > 0)
        console.log(`  termos=${dim(terms.map((term) => term.value).join(', '))}`);
    if (query.inferredFromMs !== null) {
    ...
    if (candidates.length === 0) {
        ...
        for (let i = 0; i < candidates.length; i += 1) {
            ...
            if (candidate.matchedTerms.length > 0)
                console.log(`      ${dim('match:')} ${candidate.matchedTerms.join(', ')}`);
    ...
    if (query.action === 'capture') {
        if (!selected.sessionId)
            throw new Error('nenhuma sessão encontrada para `--capture`');
        return cmdCapture([selected.sessionId, '--focus', '--behind']);
    }
    if (query.action === 'watch') {
        if (!selected.sessionId)
            throw new Error('nenhuma sessão encontrada para `--watch`');
        return cmdMonitor('watch', [selected.sessionId]);
    }
    if (query.action === 'open') {
        if (!selected.sessionId)
            throw new Error('nenhuma sessão encontrada para `--open`');
