...
function appendOpenLog(message) {
    try {
...
function buildWatchInnerCommand(target, launcher, ownerTty) {
...
function buildOpenWatchScriptCommand(target, launcher, ownerTty) {
...
function openInTmux(target, ownerTty) {
    ...
    appendOpenLog(`open_tmux_client_check status=${client.status ?? -1} client_tty=${clientTty || '(none)'} stderr=${(client.stderr ?? '').trim()}`);
    if ((client.status ?? 1) !== 0 || !clientTty)
        return false;
...
function openLinuxTerminal(target, ownerTty) {
